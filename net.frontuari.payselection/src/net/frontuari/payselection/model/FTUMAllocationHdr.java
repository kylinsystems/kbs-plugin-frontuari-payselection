...
*/
public class FTUMAllocationHdr extends MAllocationHdr implements DocAction
{
	/**
	 * 
	 */
	 ...
	 */
	public static FTUMAllocationHdr[] getOfPayment (Properties ctx, int C_Payment_ID, String trxName)
	{
		...
		ArrayList<FTUMAllocationHdr> list = new ArrayList<FTUMAllocationHdr>();
		...
		try
		{
			...
			while (rs.next())
				list.add (new FTUMAllocationHdr(ctx, rs, trxName));
		}
		catch (Exception e)
		{
		...
		FTUMAllocationHdr[] retValue = new FTUMAllocationHdr[list.size()];
	 ...
	 *	@param trxName transaction
	 */
	public static FTUMAllocationHdr[] getOfInvoice (Properties ctx, int C_Invoice_ID, String trxName)
	{
		String sql = "SELECT * FROM C_AllocationHdr h "
			+ "WHERE IsActive='Y'"
			+ " AND EXISTS (SELECT * FROM C_AllocationLine l "
	 ...
	 *	@param trxName transaction
	 */
	public static FTUMAllocationHdr[] getOfCash (Properties ctx, int C_Cash_ID, String trxName)
	{
		...
		List<FTUMAllocationHdr> list = query.list();
		FTUMAllocationHdr[] retValue = new FTUMAllocationHdr[list.size()];
		list.toArray(retValue);
		return retValue;
	}	//	getOfCash
	...
	/**	Logger						*/
	private static CLogger s_log = CLogger.getCLogger(FTUMAllocationHdr.class);
	 ...
	 *	@param trxName transaction
	 */
	public FTUMAllocationHdr (Properties ctx, int C_AllocationHdr_ID, String trxName)
	{
	 ...
	 *	@param trxName transaction
	 */
	public FTUMAllocationHdr (Properties ctx, boolean IsManual, Timestamp DateTrx, 
	 ...
	 *	@param trxName transaction
	 */
	public FTUMAllocationHdr (Properties ctx, ResultSet rs, String trxName)
	{
		super(ctx, rs, trxName);
	}	//	MAllocation

	/**	Lines						*/
	private FTUMAllocationLine[]	m_lines = null;
	 ...
	 *	@return lines
	 */
	public FTUMAllocationLine[] getLines (boolean requery)
	{
		...
		ArrayList<FTUMAllocationLine> list = new ArrayList<FTUMAllocationLine>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
			...
			while (rs.next ())
			{
				FTUMAllocationLine line = new FTUMAllocationLine(getCtx(), rs, get_TrxName());
				line.setParent(this);
				list.add (line);
			}
		} 
		...
		//
		m_lines = new FTUMAllocationLine[list.size ()];
	...
	protected boolean beforeDelete ()
	{
		...
		{
			FTUMAllocationLine line = m_lines[i];
	...
	public String prepareIt()
	{
		...
		if (!isReversal()) {
			for (FTUMAllocationLine line :m_lines)
			{	
		...
		for (int i = 0; i < m_lines.length; i++)
		{
			...
			if (line.getC_Invoice_ID() > 0) {
				...
				System.out.println(getDateAcct());
			...
			if (line.getC_Payment_ID() > 0) {
				Timestamp DatePay =TimeUtil.trunc(line.getC_Invoice().getDateAcct(), TimeUtil.TRUNC_DAY);
				System.out.println(getDateAcct());
				System.out.println(DatePay);
	...
	public boolean voidIt()
	{
			...
			|| DOCSTATUS_NotApproved.equals(getDocStatus()) )
		{
			...
			//	Set lines to 0
			FTUMAllocationLine[] lines = getLines(true);
			if(!updateBP(true))
				return false;
			
			for (int i = 0; i < lines.length; i++)
			{
				FTUMAllocationLine line = lines[i];
	...
	private boolean reverseIt(boolean accrual) 
	{
		...
		{
			//	Deep Copy
			FTUMAllocationHdr reversal = copyFrom (this, reversalDate, reversalDate, get_TrxName());
			if (reversal == null)
			{
			...
			//	Reverse Line Amt
			FTUMAllocationLine[] rLines = reversal.getLines(false);
			for (FTUMAllocationLine rLine : rLines) {
		...
		else
		{
			...
			//	Delete Posting
			MFactAcct.deleteEx(FTUMAllocationHdr.Table_ID, getC_AllocationHdr_ID(), get_TrxName());
	...
	private boolean updateBP(boolean reverse)
	{
		...
		for (FTUMAllocationLine line : m_lines) {
			...
			if (C_Payment_ID > 0)
			{
				...
				payment = new FTUMPayment (getCtx(), C_Payment_ID, get_TrxName());
	 ...
	 *	@return Allocation
	 */
	public static FTUMAllocationHdr copyFrom (FTUMAllocationHdr from, Timestamp dateAcct, Timestamp dateTrx,
		String trxName)
	{
		FTUMAllocationHdr to = new FTUMAllocationHdr (from.getCtx(), 0, trxName);
	 ...
	 */
	public int copyLinesFrom (FTUMAllocationHdr otherAllocation)
	{
		if (isProcessed() || isPosted() || (otherAllocation == null))
			return 0;
		FTUMAllocationLine[] fromLines = otherAllocation.getLines(false);
		int count = 0;
		for (FTUMAllocationLine fromLine : fromLines) {
			FTUMAllocationLine line = new FTUMAllocationLine (getCtx(), 0, get_TrxName());
			...
			if (line.getC_Payment_ID() != 0)
			{
				FTUMPayment payment = new FTUMPayment(getCtx(), line.getC_Payment_ID(), get_TrxName());
				if (DOCSTATUS_Reversed.equals(payment.getDocStatus()))
				{
					FTUMPayment reversal = (FTUMPayment) payment.getReversal();
					if (reversal != null)
					{
