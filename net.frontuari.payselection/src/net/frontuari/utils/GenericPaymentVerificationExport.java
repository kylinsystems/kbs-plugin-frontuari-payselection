 ...
 */
public class GenericPaymentVerificationExport implements PaymentVerificationExport
{
	/** Logger								*/
	static private CLogger	s_log = CLogger.getCLogger (GenericPaymentVerificationExport.class);
		...
		try
		{
			...
			for (int i = 0; i < payments.length; i++)
			{
				...
				if (payment == null)
					continue;
				//  BPartner Info
				String bp[] = getBPartnerInfo(payment.getC_BPartner_ID());
				...
				line.append(x).append(bp[BP_VALUE]).append(x).append(",")   // Value
					...
					//  Payment Info
					.append(x).append(payment.getDocumentNo()).append(x).append(",")    // DocumentNo
					.append(payment.getDateAcct()).append(",")               // PayDate
					.append(x).append(MCurrency.getISO_Code(Env.getCtx(), payment.getC_Currency_ID())).append(x).append(",")    // Currency
					.append(payment.getPayAmt()).append(",")                // PayAmount
