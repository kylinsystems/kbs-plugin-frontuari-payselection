 ...
 * Export class for Bancaribe Bank
 */
public class B_BancaribePE implements PaymentExport {

	/** Logger								*/
	static private CLogger	s_log = CLogger.getCLogger (B_BancaribePE.class);
	...
	public int exportToFile (MPaySelectionCheck[] checks, File file, StringBuffer err)
	{
		...
		try
		{
			...
			for (int i = 0; i < checks.length; i++)
			{
				...
				//	Payment Amount
				String p_Amt = String.format(Locale.US,"%1$,.2f", mpp.getPayAmt().abs());
				//	Used the variable p_ISO_Code
				String p_BP_Name = String.format("%1$-"+ 35 +"s",bp[BPA_A_NAME]);
				line = new StringBuffer();
				
				line.append(p_BP_BankAccount)												//  BP Bank Account
					.append(String.format("%1$-" + 1 + "s","/"))							//	Slace	
