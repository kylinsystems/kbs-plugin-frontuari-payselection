 ...
 * Export class for Exterior Bank
 */
public class B_ExteriorPE implements PaymentExport {

	/** Logger								*/
	static private CLogger	s_log = CLogger.getCLogger (B_ExteriorPE.class);
	...
	public int exportToFile (MPaySelectionCheck[] checks, File file, StringBuffer err)
	{
		...
		try
		{
			...
			totalAmt = String.format("%1$" + 13 + "s", totalAmt).replace(" ", "0");
			
			//  write header
			line = new StringBuffer();
			//	Header
			line.append(orgTaxID)									//	Document No
